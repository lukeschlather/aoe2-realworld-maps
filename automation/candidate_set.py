...
def window(lon: float, lat: float, span: float, rotate: float = 0.0) -> list[str]:
    args = [f"--center={lon},{lat}", "--span-km", f"{span:g}"]
    if rotate:
        args += ["--rotate", f"{rotate:g}"]
    return args
...
CANDIDATES: list[tuple[str, list[str]]] = [
    ...
    ("Michigan", [*window(-85.0, 44.5, 1200, 45), *PUGET]),

    # --- the rest of the Great Lakes set, to see how they render ---------
    ("GL current rotate0", [*window(-84.0, 44.5, 1600, 0), *PUGET]),
    ("GL northup 1600", [*window(-84.0, 44.5, 1600, 45), *PUGET]),
    ("GL northup 1400", [*window(-84.0, 44.5, 1400, 45), *PUGET]),
    ("GL northup 1200", [*window(-83.5, 44.5, 1200, 45), *PUGET]),
    ("GL northup 1000", [*window(-83.0, 45.0, 1000, 45), *PUGET]),
    ("GL five lakes 1800", [*window(-85.0, 45.5, 1800, 45), *PUGET]),
    ("GL Huron-Erie", [*window(-82.0, 44.0, 1100, 45), *PUGET]),
    ("GL Erie-Ontario", [*window(-79.5, 43.2, 900, 45), *PUGET]),
    ("GL Superior", [*window(-87.5, 47.5, 1200, 45), *PUGET]),
    ...
    ("Britain northup", [*window(-3.0, 54.5, 1300, 45), *FOREST_SPLIT]),
    ...
    ("Britain northup France", [*window(-3.0, 52.5, 1300, 45), *FOREST_SPLIT]),

    # --- proposed replacements ------------------------------------------
    ("Korea", window(127.0, 36.5, 1200, 45)),
    ("Florida", window(-83.0, 27.5, 1700, 45)),
]
