...
System A, and therefore our best template (see the full System A section
below).
...
`real_world_manchuria.rms` (see below - the decisive reference), and
...
are layered on top). That is structurally the same problem we have. Its
complete tightening:
...
Follow `real_world_manchuria.rms`, not `Arabia.rms` - it is the stock
script solving our exact problem:
...
3. `town_centres.inc`, `villagers.inc`, `scouts.inc`, `stragglers.inc`.
4. PRIMARY-only forage/gold/stone for narrow regions (Britain, Caribbean,
   Japan, Greece, New Zealand, Salish Sea); add SECONDARY/TERTIARY only for
   the roomy ones (Italy, Black Sea, Chesapeake, Scandinavia). This is the
   System A replacement for the `--tight-resources` flag, and it should
   *replace* the additive backstop rather than stack with it.
...
7. `relics.inc`, then `remote_resources.inc` unconditionally.

Per `CLAUDE.md`, the tightening choice per region needs real-engine
captures to judge, not a synthetic preview - and the zero-of-a-kind check
is the only pass/fail metric here; everything else is a fact to surface.
